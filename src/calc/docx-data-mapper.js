...
export function buildDocxPayload(params) {
  ...
  return {
    ...
    // ── ACM EN 15316 detaliat (Sprint 1) ──
    acm_consum_level: acm?.consumptionLevel || "med",
    acm_tsupply: acm?.tSupply ?? "55",
    acm_insul_class: acm?.insulationClass || "B",
    acm_volum_stocare: acm?.storageVolume ?? "",
    acm_lungime_conducte: acm?.pipeLength ?? "",
    acm_izolatie_conducte: acm?.pipeInsulationThickness || (acm?.pipeInsulated === false ? "fara" : "20mm"),
    acm_recirculare: acm?.circRecirculation ? "Da" : "Nu",
    acm_pompa_circ: acm?.circPumpType || "",
    ...
    // ── Rezultate ACM EN 15316 detaliat (Sprint 1) ──
    acm_q_nd:                instSummary?.acmDetailed?.Q_nd_annual_kWh != null ? String(instSummary.acmDetailed.Q_nd_annual_kWh) : "",
    acm_q_dist:              instSummary?.acmDetailed?.Q_dist_kWh != null      ? String(instSummary.acmDetailed.Q_dist_kWh)      : "",
    acm_q_dist_pct:          instSummary?.acmDetailed?.f_dist_pct != null      ? String(instSummary.acmDetailed.f_dist_pct)      : "",
    acm_q_storage:           instSummary?.acmDetailed?.Q_storage_kWh != null   ? String(instSummary.acmDetailed.Q_storage_kWh)   : "",
    acm_q_storage_pct:       instSummary?.acmDetailed?.f_storage_pct != null   ? String(instSummary.acmDetailed.f_storage_pct)   : "",
    acm_q_final:             instSummary?.acmDetailed?.Q_final_kWh != null     ? String(instSummary.acmDetailed.Q_final_kWh)     : "",
    acm_eta_system:          instSummary?.acmDetailed?.eta_system != null      ? instSummary.acmDetailed.eta_system.toFixed(2)   : "",
    acm_solar_fraction_pct:  instSummary?.acmDetailed?.solarFraction_pct != null ? String(instSummary.acmDetailed.solarFraction_pct) : "0",
    acm_q_solar:             instSummary?.acmDetailed?.Q_solar_kWh != null     ? String(instSummary.acmDetailed.Q_solar_kWh)     : "0",
    acm_tcold:               instSummary?.acmDetailed?.tCold != null           ? String(instSummary.acmDetailed.tCold)           : "",
    acm_q_specific_L:        instSummary?.acmDetailed?.q_specific_L != null    ? String(instSummary.acmDetailed.q_specific_L)    : "",
    acm_vol_L_calc:          instSummary?.acmDetailed?.vol_L != null           ? String(instSummary.acmDetailed.vol_L)           : "",
    acm_verdict:             instSummary?.acmDetailed?.verdict || "",
    acm_reference:           instSummary?.acmDetailed?.reference || "SR EN 15316-3/5:2017",
