        ...
        {(() => {
          ...
          const renderCard = (p) => (
            <div key={p.id} style={{ padding: "28px", borderRadius: "16px", background: p.highlight ? "rgba(245,158,11,0.05)" : cardBg, border: p.highlight ? "2px solid rgba(245,158,11,0.3)" : `1px solid ${cardBorder}`, boxShadow: p.highlight ? (isDark ? "0 4px 32px rgba(245,158,11,0.10)" : "0 4px 24px rgba(245,158,11,0.12)") : cardShadow, position: "relative" }}>
              ...
              {p.id === "audit" && <div style={{ position: "absolute", top: "-12px", left: "50%", transform: "translateX(-50%)", padding: "2px 12px", borderRadius: "10px", background: "#3b82f6", color: "#fff", fontSize: "11px", fontWeight: "700", whiteSpace: "nowrap" }}>{lang === "EN" ? "LOW VOLUME" : "VOLUM REDUS"}</div>}
              {p.id === "expert" && <div style={{ position: "absolute", top: "-12px", left: "50%", transform: "translateX(-50%)", padding: "2px 12px", borderRadius: "10px", background: "#8b5cf6", color: "#fff", fontSize: "11px", fontWeight: "700", whiteSpace: "nowrap" }}>{lang === "EN" ? "IN-DEPTH AUDIT" : "AUDIT APROFUNDAT"}</div>}
