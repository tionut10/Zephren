              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(380px, 1fr))", gap: "24px", maxWidth: "900px", margin: "0 auto" }}>
                {PLANS.filter(p => PLAN_LAYOUT.team.includes(p.id)).map(p => (
                  <div key={p.id} style={{ padding: "28px", borderRadius: "16px", background: cardBg, border: `1px solid ${cardBorder}`, position: "relative" }}>
                    ...
                    {p.id === "enterprise" && <div style={{ position: "absolute", top: "-12px", left: "50%", transform: "translateX(-50%)", padding: "2px 12px", borderRadius: "10px", background: "#dc2626", color: "#fff", fontSize: "11px", fontWeight: "700", whiteSpace: "nowrap" }}>ENTERPRISE</div>}
                    ...
                    <div style={{ margin: "14px 0" }}>
                      ...
                      {p.period && <span style={{ fontSize: "13px", color: textFaint }}>{" RON"}{p.period}</span>}
                      {p.tierNote && <div style={{ fontSize: "11px", color: textFaint, marginTop: "4px" }}>{p.tierNote}</div>}
                      {p.priceAn && <div style={{ fontSize: "11px", color: textFaint, marginTop: "4px" }}>{p.priceAn} <span style={{ color: "#f59e0b" }}>({p.discount})</span></div>}
                    </div>
                    <ul style={{ listStyle: "none", padding: 0, margin: "20px 0" }}>
                      {p.features.map(f => (
                        <li key={f} style={{ fontSize: "13px", padding: "5px 0", color: textMuted, display: "flex", alignItems: "flex-start", gap: "8px" }}>
